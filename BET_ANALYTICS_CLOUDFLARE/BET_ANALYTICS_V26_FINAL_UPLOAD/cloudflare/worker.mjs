...
export default {
  async fetch(request, env){
    ...
    if(url.pathname==='/api/version')return json({app:'BET ANALYTICS',version:'26.0.0',parserVersion:PARSER_VERSION});
    if(url.pathname==='/api/football')return footballApi(request);
    if(url.pathname==='/api/leagues')return json({version:PARSER_VERSION,leagues:catalog()});
