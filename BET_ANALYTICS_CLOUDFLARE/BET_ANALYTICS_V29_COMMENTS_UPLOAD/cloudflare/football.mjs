export const PARSER_VERSION='29.0.0';
...
// V28: whitelist stricte. Une D1 quelconque n'est PLUS acceptée automatiquement.
// L'objectif est d'éviter D1 Algérie / D1 Colombie / etc. tout en gardant les
// principaux championnats masculins et compétitions continentales demandées.
const MAJOR_COMPETITIONS=[
  ...
  ['turkey-super-lig','Turquie Süper Lig',/\bD1\s+Turquie\b/i],
  ['netherlands-eredivisie','Pays-Bas Eredivisie',/\bD1\s+Pays-Bas\b/i],
  ['belgium-pro-league','Belgique Pro League',/\bD1\s+Belgique\b/i],
  ['scotland-premiership','Écosse Premiership',/\bD1\s+Ecosse\b|\bD1\s+Écosse\b/i],
  ['switzerland-super-league','Suisse Super League',/\bD1\s+Suisse\b/i],
  ['austria-bundesliga','Autriche Bundesliga',/\bD1\s+Autriche\b/i],
  ['denmark-superliga','Danemark Superliga',/\bD1\s+Danemark\b/i],
  ['norway-eliteserien','Norvège Eliteserien',/\bD1\s+Norv[eè]ge\b/i],
  ['sweden-allsvenskan','Suède Allsvenskan',/\bD1\s+Su[eè]de\b/i],
  ['finland-veikkausliiga','Finlande Veikkausliiga',/\bD1\s+Finlande\b/i],
  ['poland-ekstraklasa','Pologne Ekstraklasa',/\bD1\s+Pologne\b/i],

  ['mls','MLS',/\bMLS\b/i],
  ['brazil-serie-a','Brésil Serie A',/\bD1\s+Br[eé]sil\b/i],
  ['argentina-primera','Argentine Primera División',/\bD1\s+Argentine\b/i],
  ['mexico-liga-mx','Mexique Liga MX',/\bD1[.\s]+Mexique\b/i],
  ['japan-j1','Japon J1 League',/\bD1\s+Japon\b/i],
  ['korea-k1','Corée du Sud K League 1',/\bD1\s+Cor[eé]e\s+du\s+Sud\b/i],
  ['china-csl','Chine CSL',/\bD1\s+Chine\b/i],
  ['saudi-pro','Arabie saoudite Pro League',/\bD1\s+Ar[.]?Saoudite\b|\bD1\s+Arabie\s+Saoudite\b/i],
  ['australia-a-league','Australie A-League',/\bD1\s+Australie\b/i],
...
const EXCLUDED_COMP=/\b(?:D2|Ligue 2|LaLiga 2|Bundesliga 2|Championship|EFL Cup|Comm\.? Shield|Super Coupe|UEFA Super Cup|Troph\.?Champions|Cpe Ligue|Coupe|Cup|Serie B)\b/i;
...
function noVig1X2(o){const h=Number(o?.home),d=Number(o?.draw),a=Number(o?.away);if(!(h>1&&d>1&&a>1))return null;const x=[1/h,1/d,1/a],z=x.reduce((p,q)=>p+q,0);return {home:+(100*x[0]/z).toFixed(1),draw:+(100*x[1]/z).toFixed(1),away:+(100*x[2]/z).toFixed(1)}}
...
export function classifyCompetition(raw){
  ...
  for(const [key,canonical,re] of MAJOR_COMPETITIONS){if(re.test(name))return {accepted:true,reason:'major-men-whitelist',key,canonical}}
  return {accepted:false,reason:'not-in-major-whitelist'};
}

function extractCompetition(segment){
  const candidates=[];
  for(const [key,canonical,re] of MAJOR_COMPETITIONS){const m=segment.match(re);if(m)candidates.push({index:m.index,text:m[0],key,canonical})}
...
export function parsePronosoftHtml(html,{year=currentYearZurich()}={}){
  ...
  for(let i=0;i<times.length;i++){
    ...
    fixtures.push({
      ...
      home:{name:home},away:{name:away},odds,source:'Pronosoft ParionsSport 1N2',feedSources:['Pronosoft'],synthesis:{oneXtwo:noVig1X2(odds)}
    });
  }
...
async function fetchText(url){
  try{
    const r=await fetch(url,{headers:{'user-agent':'Mozilla/5.0 (compatible; BET-ANALYTICS/29.0; +https://bet-analytics.ckenduz.workers.dev)','accept':'text/html,application/xhtml+xml'}});
...
export async function footballApi(request){
  ...
  const body={version:PARSER_VERSION,parserVersion:PARSER_VERSION,date:requested,results:fixtures.length,fixtures,leagueCounts,availableDates,source:'Pronosoft public ParionsSport 1N2',sourceUrl:SOURCE_URL,sourceStatus:[{url:src.url,ok:true,bytes:src.bytes,contentType:src.contentType}],filter:'strict-major-men-whitelist'};
  if(debug)body.diagnostics={rawAcceptedFixtures:parsed.fixtures.length,dateHeadings:parsed.dateMarks,rejected:parsed.rejected.slice(0,60),firstFixtures:parsed.fixtures.slice(0,8)};
  return json(body);
}
export function catalog(){return MAJOR_COMPETITIONS.map(([key,name])=>({key,name}))}
