...
export default async function scorerPremium(req,env={}){
  ...
  const key=getKey(env);if(!key)return Response.json({available:false,error:'API_FOOTBALL_KEY non configurée',message:'Le module Buteur Premium est installé mais reste volontairement vide sans source joueur vérifiée. Aucune donnée n’est inventée.'},{status:200});
  try{
