...
function App(){
  ...
  <div className="leaguebar"><button className={leagueFilter==='ALL'?'active':''} onClick={()=>setLeagueFilter('ALL')}>Toutes</button>{[...new Set(fixtures.map(x=>x.league?.name).filter(Boolean))].map(l=><button key={l} className={leagueFilter===l?'active':''} onClick={()=>setLeagueFilter(l)}>{l} · {fixtures.filter(g=>g.league?.name===l).length}</button>)}</div>
