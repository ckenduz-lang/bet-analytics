...
function App(){
  ...
  <div className="leagueTitle"><b>Ligues hommes majeures</b><span>Liste dynamique Maçkolik · féminin, U21/U19, réserves et divisions inférieures exclus.</span></div><div className="leaguebar"><button className={leagueFilter==='ALL'?'active':''} onClick={()=>setLeagueFilter('ALL')}>Toutes · {fixtures.length}</button>{leagues.map(l=>{const n=fixtures.filter(g=>g.league?.name===l).length;return <button key={l} className={leagueFilter===l?'active':''} onClick={()=>setLeagueFilter(l)}>{l} · {n}</button>})}</div>
